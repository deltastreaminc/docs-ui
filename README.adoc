...
== Use the Default UI theme bundle
...
== Use our custom UI theme bundle

To use the custom DeltaStream UI theme with Antora, add the following configuration to your playbook file:

NOTE: Final URL TBD and could be subject to changes. How are we going to handle bundle releases and the URL?
We might have to make the ui theme repo public. Need to test pointing to private repo and our bundle when available. 
the `git@` segment in a URL indicates which repositories are private (`url: https://git@github.com/`). Additional tokens are required in this scenario.

[source,yaml]
----
ui:
  bundle:
    url: https://github.com/deltastreaminc/docs-ui/ui-bundle.zip
----
...
[source,yaml]
----
https://github.com/couchbase/docs-ui/releases/download/prod-144/ui-bundle.zip
----
...
Replace `%UI_BUNDLE_URL%` with desired release bundle. See https://github.com/mulesoft/docs-site-ui for more info and their examples.
